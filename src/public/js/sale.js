...
select_product.addEventListener('change',(event)=>{
    let price=select_product.options[select_product.selectedIndex].value
...
function insertNewRow(data){
    ...
    cell2.innerHTML=data['pricePerUnit']
    cell3.innerHTML=data['unit']
    cell4.innerHTML=data['price']
    cell5.innerHTML=`<a class="mx-2 btn btn-outline-warning" onClick="onEdit(this)"><i class="bi bi-pencil-fill"></i></a>
                    <a class="mx-2 btn btn-outline-danger" onClick="onDelete(this)"><i class="bi bi-x-square-fill"></i></a>`
}

function updateRow(data){
    selectedRow.cells[0].innerHTML=data['product']
    selectedRow.cells[1].innerHTML=data['pricePerUnit']
    selectedRow.cells[2].innerHTML=data['unit']
...
function onEdit(td){
    ...
    let product=selectedRow.cells[0].innerHTML
    let pricePerUnit=selectedRow.cells[1].innerHTML
    let unit=selectedRow.cells[2].innerHTML
...
function prepareForSendData(){
    ...
    for( let i=0; i<length; i++){
        ...
        itemPrice[i]=table.rows[i].cells[1].innerHTML
        itemUnit[i]=table.rows[i].cells[2].innerHTML
        itemTotal[i]=table.rows[i].cells[3].innerHTML
    }
    in_item.value=item
